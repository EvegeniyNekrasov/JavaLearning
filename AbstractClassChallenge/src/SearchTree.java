    ...
    private void performRemoval(ListItem item, ListItem parent) {
        ...
        if (item.next() == null) {
            ...
            if (parent.next() == item) {
                ...
                // parent must be item, which means we were to right tree (which may be null)
            ...
            // From the right sub-three, find the smallest value (i.e., the leftmost)
            ...
            if (leftmostParent == item) {
                // there was no leftmost node, so 'current' points to be smallest
                // node (the one that must now be deleted)
                item.setValue(current.next());
            } else {
                // set smallest node's parent to point to
